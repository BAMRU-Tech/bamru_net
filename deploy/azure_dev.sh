...
if $SETUP_DB; then
    ...
    CONNECT_NEW="$CONNECT --username=${POSTGRES_USER}@${POSTGRES_NAME}"
    PGSSLMODE=require
    PGPASSWORD="$POSTGRES_PASSWORD" psql $CONNECT_ADMIN ${POSTGRES_DB} <<EOF
    CREATE DATABASE ${SLOT_POSTGRES_DB};
    CREATE ROLE ${SLOT_POSTGRES_USER} WITH LOGIN NOSUPERUSER INHERIT NOCREATEDB NOCREATEROLE NOREPLICATION PASSWORD '${SLOT_POSTGRES_PASSWORD}';
    GRANT CONNECT ON DATABASE ${SLOT_POSTGRES_DB} TO ${SLOT_POSTGRES_USER};
    GRANT ALL PRIVILEGES ON DATABASE ${SLOT_POSTGRES_DB} TO ${SLOT_POSTGRES_USER};
EOF
    PGPASSWORD="$POSTGRES_PASSWORD" pg_dump --format=custom --file=db.dump $CONNECT_ADMIN --dbname=$POSTGRES_DB
    PGPASSWORD="$POSTGRES_PASSWORD" pg_restore $CONNECT_NEW -d ${SLOT_POSTGRES_DB} db.dump
fi
