...
var adata=dt.rows({selected:!0});var data="";var multiSelect=this.s.dt.rows({selected:!0})[0].length>1;data+="<form name='altEditor-form' role='form'>";data+='<input type="hidden" id="tblId" value="'+dt.table().node().id+'">';for(var j in columnDefs){if(columnDefs[j].type.includes("hidden")||(multiSelect&&!columnDefs[j].type.includes("allowMultiSelect")))
...
data=response;else data=JSON.parse(response);$('#altEditor-modal .modal-body .alert').remove();var message='<div class="alert alert-success" role="alert">'+'<strong>Success!</strong>'+'</div>';$('#altEditor-modal .modal-body').append(message);setTimeout(function(){$('#altEditor-modal').modal('hide')},1000);this.s.dt.row({selected:!0}).data(data);this.s.dt.rows({selected:!0}).deselect();this.s.dt.draw();$("div#altEditor-modal").find("button#addRowBtn").prop('disabled',!0);$("div#altEditor-modal").find("button#editRowBtn").prop('disabled',!0);$("div#altEditor-modal").find("button#deleteRowBtn").prop('disabled',!0)},_errorCallback:function(response,status,more){var error=response;$('#altEditor-modal .modal-body .alert').remove();var errstr="There was an unknown error!";if(error.responseJSON&&error.responseJSON.errors){errstr="";for(var key in error.responseJSON.errors){errstr+=error.responseJSON.errors[key][0]}}
