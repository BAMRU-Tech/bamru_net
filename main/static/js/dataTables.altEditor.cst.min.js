...
var adata=dt.rows({selected:!0});var data="";var multiSelect=this.s.dt.cells('.marked').count()>1;data+="<form name='altEditor-form' role='form'>";data+='<input type="hidden" id="tblId" value="'+dt.table().node().id+'">';for(var j in columnDefs){if(columnDefs[j].type.includes("hidden")||(multiSelect&&!columnDefs[j].type.includes("allowMultiSelect")))
...
if(columnDefs[j].type.includes("date")){data+="<input type='date' pattern='[0-9]{4}-[0-9]{2}-[0-9]{2}'"+" oninvalid='this.setCustomValidity(\"Enter valid date yyyy-mm-dd\")' id='"+columnDefs[j].name+"' name='"+columnDefs[j].title+"' placeholder='"+columnDefs[j].title+"' style='overflow:hidden'  class='form-control  form-control-sm' value='"+(!multiSelect?adata.data()[0][columnDefs[j].name]:'')+"'>"}
if(columnDefs[j].type.includes("time")){data+="<input type='time' pattern='[0-9]{2}:[0-9]{2}'"+" oninvalid='this.setCustomValidity(\"Enter valid time 24H hh:mm\")' id='"+columnDefs[j].name+"' name='"+columnDefs[j].title+"' placeholder='"+columnDefs[j].title+"' style='overflow:hidden'  class='form-control  form-control-sm' value='"+(!multiSelect?adata.data()[0][columnDefs[j].name]:'')+"'>"}
...
var adata=dt.rows({selected:!0});var data="";data+="<form name='altEditor-form' role='form'>";data+='<input type="hidden" id="tblId" value="'+dt.table().node().id+'">';for(var j in columnDefs){if(columnDefs[j].type.includes("hidden")){data+="<input type='hidden' id='"+columnDefs[j].title+"' value='"+adata.data()[0][columnDefs[j].name]+"'>"}
else{data+="<div style='margin-left: initial;margin-right: initial;' class='form-group row'><label for='"+that._quoteattr(columnDefs[j].title)+"'>"+columnDefs[j].title+":  </label> <input  type='hidden'  id='"+that._quoteattr(columnDefs[j].title)+"' name='"+that._quoteattr(columnDefs[j].title)+"' placeholder='"+that._quoteattr(columnDefs[j].title)+"' style='overflow:hidden'  class='form-control' value='"+that._quoteattr(adata.data()[0][columnDefs[j].name])+"' >"+adata.data()[0][columnDefs[j].name]+"</div>"}}
...
if(columnDefs[j].type.includes("date")){data+="<input type='date' pattern='[0-9]{4}-[0-9]{2}-[0-9]{2}'"+" oninvalid='this.setCustomValidity(\"Enter valid date yyyy-mm-dd\")' id='"+columnDefs[j].name+"' name='"+columnDefs[j].title+"' placeholder='"+columnDefs[j].title+"' style='overflow:hidden'  class='form-control  form-control-sm' value=''>"}
if(columnDefs[j].type.includes("time")){data+="<input type='time' pattern='[0-9]{2}:[0-9]{2}'"+" oninvalid='this.setCustomValidity(\"Enter valid time 24H hh:mm\")' id='"+columnDefs[j].name+"' name='"+columnDefs[j].title+"' placeholder='"+columnDefs[j].title+"' style='overflow:hidden'  class='form-control  form-control-sm' value=''>"}
...
data=response;else data=JSON.parse(response);$('#altEditor-modal .modal-body .alert').remove();var message='<div class="alert alert-success" role="alert">'+'<strong>Success!</strong>'+'</div>';$('#altEditor-modal .modal-body').append(message);setTimeout(function(){$('#altEditor-modal').modal('hide')},1000);var multiSelect=this.s.dt.cells('.marked').count()>1;if(!multiSelect)this.s.dt.row({selected:!0}).data(data);this.s.dt.rows({selected:!0}).deselect();this.s.dt.draw();$("div#altEditor-modal").find("button#addRowBtn").prop('disabled',!0);$("div#altEditor-modal").find("button#editRowBtn").prop('disabled',!0);$("div#altEditor-modal").find("button#deleteRowBtn").prop('disabled',!0)},_errorCallback:function(response,status,more){var error=response;$('#altEditor-modal .modal-body .alert').remove();var errstr="There was an unknown error!";if(error.responseJSON&&error.responseJSON.errors){errstr="";for(var key in error.responseJSON.errors){errstr+=error.responseJSON.errors[key][0]}}
...
return isValid},_quoteattr:function(s,preserveCR){preserveCR=preserveCR?'&#13;':'\n';return(''+s).replace(/&/g,'&amp;').replace(/'/g,'&apos;').replace(/"/g,'&quot;').replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(/\r\n/g,preserveCR).replace(/[\r\n]/g,preserveCR)},});altEditor.version='2.0.1';altEditor.defaults={alwaysAsk:!1,focus:null,columns:'',update:null,editor:null};altEditor.classes={btn:'btn'};$(document).on('preInit.dt.altEditor',function(e,settings,json){if(e.namespace!=='dt'){return}
